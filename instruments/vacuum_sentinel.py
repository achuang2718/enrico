...
SLACK_ERROR_REUPDATE_TIME_SECS = 600
...
def main():
	...
	while(True):
		...
		if(current_time - old_time > DELAY_TIME or current_time - old_time < 0):
			...
			counter += 1
			readings_dict, errors_list, thresholds_list = my_monitor.monitor_once()
			if(PRINT_VALUES):
				print(readings_dict) 
			if(len(errors_list) != 0):
				...
				if(error_count < ERROR_PATIENCE and error_fault):
					error_fault = False 
					my_monitor.warn_on_slack("The outstanding reading error has been resolved.")
			if(len(thresholds_list) != 0):
				...
				if(threshold_count < THRESHOLD_PATIENCE and threshold_fault):
					threshold_fault = False
					my_monitor.warn_on_slack("The outstanding threshold warning has been resolved. All vacuum values below threshold.") 
